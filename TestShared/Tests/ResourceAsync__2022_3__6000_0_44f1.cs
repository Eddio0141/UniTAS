...
public class ResourceAsync__2022_3__6000_0_44f1 : MonoBehaviour
{
    ...
    [TestInjectResource(nameof(emptyObjectAsset))]
    public OnceOnlyPath sceneAsyncRelationResource;
    [TestInjectScene] public string emptyScene;

    [Test]
    public IEnumerator<TestYield> SceneAsyncRelation()
    {
        // check if there's a connection with the scene async queue with asset bundle
        // ---
        // result: no connection
        var resource = Resources.LoadAsync(sceneAsyncRelationResource);
        var scene = SceneManager.LoadSceneAsync(emptyScene);

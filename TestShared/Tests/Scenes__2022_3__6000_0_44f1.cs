...
public class Scenes__2022_3__6000_0_44f1 : MonoBehaviour
{
    ...
    {
        yield return new UnityYield(SceneManager.LoadSceneAsync(emptyScene, LoadSceneMode.Additive));
        var op = SceneManager.UnloadSceneAsync(emptyScene);
        Assert.False(op.isDone);
        yield return new UnityYield(null);
        Assert.True(op.isDone);
    }

    public Dictionary<string, ITestAsset> unloadWithSceneAssets => new()
    {
        { "scene.unity", new SceneAsset() },
    };

    [TestInjectAssetBundle(nameof(unloadWithSceneAssets))]
    public OnceOnlyPath unloadWithSceneBundle;

    [Test(InitTestTiming.Awake)]
    public IEnumerator<TestYield> UnloadAssetBundleWithScene()
    {
        var op = AssetBundle.LoadFromFileAsync(unloadWithSceneBundle);
        yield return new UnityYield(op);
        Assert.True(op.isDone);

        var scenes = op.assetBundle.GetAllScenePaths();
        Assert.Equal(scenes.Length, 1);
        var scene = scenes[0];
        // unsure why DontDestroyOnLoad doesn't stay here, awake isn't fast enough
        SceneManager.LoadScene(scene, LoadSceneMode.Additive);
        yield return new UnityYield(null);
        Assert.Equal(SceneManager.GetSceneAt(1).path, scene);
        ...
        Assert.LogRecieved(LogType.Error);
    }
}

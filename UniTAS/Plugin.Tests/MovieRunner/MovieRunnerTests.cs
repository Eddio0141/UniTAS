...
using UniTAS.Plugin.Movie.Parsers;
using UniTAS.Plugin.Movie.Parsers.MoviePropertyParser;
...
{
    private static Movie.MovieRunner Setup(IEnumerable<EngineExternalMethod> getDefinedMethods)
    {
        var externMethods = getDefinedMethods.ToList();
        var runner = new Movie.MovieRunner(

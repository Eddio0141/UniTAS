...
using UniTAS.Plugin.Movie.Exceptions.ParseExceptions;
using UniTAS.Plugin.Movie.LowLevel.OpCodes;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.BitwiseOps;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.Jump;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.Logic;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.Maths;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.Method;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.RegisterSet;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.Scope;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.StackOp;
using UniTAS.Plugin.Movie.LowLevel.OpCodes.Tuple;
using UniTAS.Plugin.Movie.LowLevel.Register;
using UniTAS.Plugin.Movie.MovieModels.Script;
using UniTAS.Plugin.Movie.Parsers.MovieScriptParser.Expressions;
...
public class ScriptCompiler : MovieGrammarBaseListener
{
    private readonly EngineExternalMethod[] _externalMethods;

    public ScriptCompiler(IEnumerable<EngineExternalMethod> externMethods)
    {
    ...
    private readonly List<OpCode> _mainBuilder = new();
    ...
    private readonly Stack<List<Expression>> _expressionBuilders = new();
    ...
    private RegisterType BuildExpressionOpCodes()
    {
        ...
        var singularExpressionStorage = new Stack<Expression>();
        var i = 0;
    ...
    private void InsertOpCodeAndUpdateOffset(int index, OpCode opCode)
    {
        ...
        for (var i = 0; i < index; i++)
        {
            ...
            if (opCodePrev is not Jump jump) continue;

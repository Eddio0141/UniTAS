...
public class HookOnGameRestart(IGameRestart gameRestart, ILogger logger) : TerminalCmd
{
    ...
    private void Execute(Closure value, bool hook)
    {
        if (hook)
        {
            ...
            logger.LogDebug($"Adding hook_on_game_restart hook, {_callbacks.Count} callbacks total");
            return;
        }
        ...
        logger.LogDebug($"Removing hook_on_game_restart hook, {_callbacks.Count} callbacks total");
    }
}
